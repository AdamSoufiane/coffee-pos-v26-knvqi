...
import adapter.AdapterInventoryDeleteResponse;
...
public class ApplicationInventoryServiceException {
    ...
    public ResponseEntity<AdapterInventoryDeleteResponse> handleException(Exception e) {
        if (e instanceof DomainInventoryItemNotFoundException) {
            log.error("Inventory item not found: {}", e.getMessage(), e);
            ...
            log.error("Inventory item already exists: {}", e.getMessage(), e);
            ...
            log.error("Invalid inventory item quantity: {}", e.getMessage(), e);
